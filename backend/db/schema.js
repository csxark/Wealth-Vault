...
export const financialHealthScoresRelations = relations(financialHealthScores, ({ many }) => ({
    expenses: many(expenses),
    goals: many(goals),
    budgetAlerts: many(budgetAlerts),
    budgetAggregates: many(budgetAggregates),
...
export const forecastScenarios = pgTable('forecast_scenarios', {
    ...
    initialCashBalance: numeric('initial_cash_balance', { precision: 15, scale: 2 }).default('0'),
    minimumCashReserve: numeric('minimum_cash_reserve', { precision: 15, scale: 2 }).default('0'),
...
export const forecastSimulationResults = pgTable('forecast_simulation_results', {
    ...
    finalCashBalance: numeric('final_cash_balance', { precision: 15, scale: 2 }).notNull(),
    minCashBalance: numeric('min_cash_balance', { precision: 15, scale: 2 }).notNull(),
    maxCashBalance: numeric('max_cash_balance', { precision: 15, scale: 2 }).notNull(),
...
export const forecastAggregates = pgTable('forecast_aggregates', {
    ...
    p10FinalBalance: numeric('p10_final_balance', { precision: 15, scale: 2 }).notNull(), // 10th percentile - pessimistic
    p50FinalBalance: numeric('p50_final_balance', { precision: 15, scale: 2 }).notNull(), // 50th percentile - median
    p90FinalBalance: numeric('p90_final_balance', { precision: 15, scale: 2 }).notNull(), // 90th percentile - optimistic
    ...
    depletionProbability: numeric('depletion_probability', { precision: 5, scale: 4 }), // % of sims that depleted
    ...
    // Summary statistics
    meanFinalBalance: numeric('mean_final_balance', { precision: 15, scale: 2 }).notNull(),
    stdDevFinalBalance: numeric('std_dev_final_balance', { precision: 15, scale: 2 }).notNull(),
    skewness: doublePrecision('skewness'), // Distribution skewness
    kurtosis: doublePrecision('kurtosis'), // Distribution kurtosis (tail risk)

    // Risk metrics
    valueatRisk95: numeric('value_at_risk_95', { precision: 15, scale: 2 }), // 95% VaR
    conditionalVaR95: numeric('conditional_var_95', { precision: 15, scale: 2 }), // Expected shortfall
    maxDrawdown: numeric('max_drawdown', { precision: 15, scale: 2 }), // Worst drop from peak
...
export const runwayAlertThresholds = pgTable('runway_alert_thresholds', {
    ...
    maxDepletionProbability: numeric('max_depletion_probability', { precision: 5, scale: 4 }).default('0.20'), // Alert if >20% depletion risk
    minCashReserveP10: numeric('min_cash_reserve_p10', { precision: 15, scale: 2 }).default('5000'), // Alert if P10 balance < $5k
    ...
    circuitBreakerThreshold: numeric('circuit_breaker_threshold', { precision: 5, scale: 4 }).default('0.30'), // Trip at 30% depletion risk
...
// ============================================================================
// CRYPTOGRAPHIC SUCCESSION "DEAD-MAN'S SWITCH" & MULTI-SIG ORCHESTRATOR (#534)
// ============================================================================

export const successionPlans = pgTable('succession_plans', {
    id: uuid('id').defaultRandom().primaryKey(),
    userId: uuid('user_id').references(() => users.id, { onDelete: 'cascade' }).notNull(),
    status: text('status').default('active'), // 'active', 'grace_period', 'triggered', 'cancelled'
    ...
    // Public Record Oracle & ZKP Fallback (#783)
    oracleVerifiedDeath: boolean('oracle_verified_death').default(false),
    oracleLastCheckAt: timestamp('oracle_last_check_at'),
    zkpProofHash: text('zkp_proof_hash'), // Fingerprint of the ZKP proof used for verification

    ledgerSignature: text('ledger_signature'), // Cryptographic signature of the digital asset ledger
    metadata: jsonb('metadata').default({}),
    createdAt: timestamp('created_at').defaultNow(),
    updatedAt: timestamp('updated_at').defaultNow(),
});

export const successionHeirs = pgTable('succession_heirs', {
    id: uuid('id').defaultRandom().primaryKey(),
    planId: uuid('plan_id').references(() => successionPlans.id, { onDelete: 'cascade' }).notNull(),
    name: text('name').notNull(),
    email: text('email').notNull(),
    publicKey: text('public_key'), // Heir's PGP or cryptographic public key for shard encryption
    role: text('role').default('heir'), // 'heir', 'lawyer', 'executor'
    shardIndex: integer('shard_index'), // Shamir shard index if applicable
    metadata: jsonb('metadata').default({}),
});

export const successionAccessShards = pgTable('succession_access_shards', {
    id: uuid('id').defaultRandom().primaryKey(),
    heirId: uuid('heir_id').references(() => successionHeirs.id, { onDelete: 'cascade' }).notNull(),
    encryptedShard: text('encrypted_shard').notNull(), // The actual access fragment, encrypted for the heir
    shardType: text('shard_type').notNull(), // 'api_key', 'private_key_fragment', 'password'
    description: text('description'),
    distributedAt: timestamp('distributed_at'),
});

export const successionHeartbeats = pgTable('succession_heartbeats', {
    id: uuid('id').defaultRandom().primaryKey(),
    userId: uuid('user_id').references(() => users.id, { onDelete: 'cascade' }).notNull(),
    source: text('source').notNull(), // 'email_click', 'app_login', 'ledger_activity'
    detectedAt: timestamp('detected_at').defaultNow(),
    ipAddress: text('ip_address'),
});
...
export const successionPlansRelations = relations(successionPlans, ({ one, many }) => ({
    user: one(users, { fields: [successionPlans.userId], references: [users.id] }),
    heirs: many(successionHeirs),
}));

export const successionHeirsRelations = relations(successionHeirs, ({ one, many }) => ({
    plan: one(successionPlans, { fields: [successionHeirs.planId], references: [successionPlans.id] }),
    shards: many(successionAccessShards),
}));

export const successionAccessShardsRelations = relations(successionAccessShards, ({ one }) => ({
    heir: one(successionHeirs, { fields: [successionAccessShards.heirId], references: [successionHeirs.id] }),
}));
